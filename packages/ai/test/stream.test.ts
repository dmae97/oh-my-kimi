...
describe("Generate E2E Tests", () => {
	...
	// No gateway Workers AI block: Cloudflare deprecated the /compat endpoint it
	// used, and upstream stopped listing workers-ai/* under this provider, so no
	// catalog model routes through it. The direct Workers AI block above still
	// covers Kimi, and the gateway's OpenAI and Anthropic passthroughs are below.
	...
	describe.skipIf(!process.env.ZYLOO_API_KEY)("Zyloo Provider (Claude Opus 4.7 via OpenAI Completions)", () => {
	...
	// =========================================================================
	// OAuth-based providers (credentials from ~/.omk/agent/oauth.json)
	...
	describe.skipIf(!hasBedrockCredentials())("Amazon Bedrock Provider (claude-opus-4-6 interleaved thinking)", () => {
		...
		it("should pass requestMetadata to the SDK payload", { retry: 3 }, async () => {
			...
			let capturedPayload: unknown;
			const metadata = { app: "omk-test", env: "ci" };
			const response = await complete(
				llmSonnet,
				{
					messages: [
	...
	if (!process.env.OMK_NO_LOCAL_LLM) {
		try {
