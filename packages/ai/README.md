...
TypeBox exports are re-exported from `omk-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'omk-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'omk-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'omk-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'omk-ai';
...
```typescript
import { getImageModel, generateImages } from '@mariozechner/omk-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'omk-ai';
...
```typescript
import { getModel, stream } from 'omk-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'omk-ai';
...
```typescript
import { Model, stream } from 'omk-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are OMK thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'omk-ai';
...
```typescript
import { getModel, complete, Context } from 'omk-ai';
...
```typescript
import { Context, getModel, complete } from 'omk-ai';
...
- OAuth login flows are not supported in browser environments. Use the `omk-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'omk-ai';
...
```bash
npx omk-ai login              # interactive provider selection
npx omk-ai login anthropic    # login to specific provider
npx omk-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `omk-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from 'omk-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `omk-ai`
