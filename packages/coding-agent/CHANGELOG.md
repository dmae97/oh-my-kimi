...
- TB21 model contract (working tree, unreleased): single-model execution boundary with fail-closed vision routing, text projection for images on text-only models, `provider_request` audit events, contract-gated compaction, plus pure `deadline-policy`, `stagnation-tracker`, and `dag-barrier-waste` modules. No default-path behavior change without an explicit `modelContract`.


### Added

- The explicit advisory-selection SDK now reports submitted/eligible/excluded candidate counts and distinguishes skipped, unavailable, invalid and scored comparisons. Top-score ties retain the caller's deterministic rank but report `judge-tied` / `deterministic`; score margins are not correctness probabilities. Existing stored decisions may omit the additive diagnostics.
- Hardened first-party advisory judge completion: only a normal `stop` may supply scores, even when truncated/aborted output contains complete JSON. Cancellation is checked before and after judge work and around auth, without extra calls or retries. Deterministic-pass admission and protocol verdicts are unchanged; no default TUI judge or AdaptOrch service is activated.
...
- `/login` → Use a subscription now includes **Muse Code**. The flow is Meta's device-code login plus a daily `/muse-code/key` mint, so a Muse Code subscription authenticates `meta` / Muse Spark. HTTP(S) proxies are honored. Pasted `META_API_KEY` values remain pay-as-you-go.

- models.json accepts `compat.sendCodexTurnMetadata` on `openai-responses` providers, and the custom-provider guide documents the [codex-chatgpt-web](https://github.com/miuuyy/codex-chatgpt-web) loopback bridge that needs it to expose ChatGPT Web (`chatgpt-web/*`) models to OMK.
- `omk provider sync <provider-id>` keeps a codex-chatgpt-web provider's models.json rows in step with the launcher. The bridge's `/v1/models` document is account-aware — the same `chatgpt-web/high` slug is a 90,000-token window on Plus and 333,579 on Pro with Bigger Context — and the hand-written rows from the guide made OMK compact three to eight times too early on a Pro account. The command checks `/healthz`, fetches the catalog with OMK's `openai-codex` OAuth plus a Codex `client_version` (the backend hides models from clients it considers too old), and rewrites only the provider's `chatgpt-web/*` rows: `name`, `reasoning`, `input` follow the catalog and `contextWindow` follows the catalog's `auto_compact_token_limit` — the budget the bridge expects a client to compact within (Codex's effective window), not its rejection ceiling, because OMK applies its own compaction ratio on top; `maxTokens`, `cost`, `thinkingLevelMap`, other keys, other providers, and the file's indentation are preserved; rows the bridge no longer serves are dropped. The provider gains a `bridgeCatalog` provenance object (`bridgeVersion`, `clientVersion`, `syncedAt`, per-row `contextCeilings`) so the numbers stay explainable, and a launcher upgrade is recorded even when no row moves. `--dry-run` reports the drift without writing, `--json` emits one document, and every failure names the next step (start the launcher, `/login openai-codex`, sign in and install models). Verified against launcher 5.0.4 in full mode.
...
- Meta Model API is now a built-in provider: `META_API_KEY` (or `META_MODEL_API_KEY` / `MODEL_API_KEY`) is detected at startup, `/login` shows "Meta Model API", and `--provider meta` defaults to `muse-spark-1.3`. Muse Spark exposes thinking levels `minimal` through `max`; `max` is sent as the model's top effort tier, `xhigh`.

### Fixed

- The `modelstudio-maas` readiness default and the second safety-failover candidate now name `qwen3.8-max` instead of `qwen3.8-max-preview`. The Singapore Token Plan endpoint (`token-plan.ap-southeast-1.maas.aliyuncs.com`) no longer lists the preview id — its live `/models` catalog serves `qwen3.8-max` and `qwen3.8-flash` — so the failover chain pointed at a model the provider rejects, and `omk-initcheck` reported the pair as missing on an otherwise healthy install. The dated `qwen3.8-max-0902` snapshot (released 2026-09-02) is only served by the pay-as-you-go `dashscope-intl` endpoint and stays a models.json choice rather than a built-in default.
...
- Classified Codex ChatGPT-account unsupported-model rejections (HTTP 400 `The '<slug>' model is not supported when using Codex with a ChatGPT account.`) as the same permanent `configuration`/`invalid` fault. Same-model retry, orphan-`tool_call_id` sanitize, and `/new session` all re-send the slug and get the same 400; next-action now points at `/model` (or an API-key route that has the model) and states that a new session will not grant access.
- Classified provider-capacity bodies that carry no status or limit token — xAI `currently at capacity` / `high demand` (HTTP 429) and Anthropic `overloaded_error` / `Overloaded` (HTTP 529) — as `provider.rate_limit` rather than the protocol default, so the turn waits or switches model instead of advertising transcript sanitize.
