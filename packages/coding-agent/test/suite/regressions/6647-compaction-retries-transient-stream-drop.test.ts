...
/**
 * Regression port of upstream pi #6647: compaction ran a single non-retried
 * summarization call, so a transient mid-stream socket death (`terminated`) failed
 * the whole compaction. Summarization now reuses `settings.retry` via
 * retryAssistantCall (bounded retries with exponential backoff gated on
 * isRetryableAssistantError), emits `summarization_retry_*` events, and
 * aborts / non-retryable errors are not retried.
 */
