...
Content`,
		...
		it("should auto-discover root markdown skills from .omk skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .omk", async () => {
			const extDir = join(tempDir, ".omk", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".omk", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".omk", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".omk", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".omk", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".omk", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".omk", "agent");
		...
		it("should dedupe user skill entries when ~/.omk/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .omk auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".omk\n");

			const skillDir = join(tempDir, ".omk", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.omk\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".omk", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .omk settings base", () => {
			...
			const rel = relative(join(tempDir, ".omk"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".omk", "npm", "node_modules", "example");
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".omk", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".omk", "npm", "node_modules", "project-current");
		...
		it("should reinstall OMKnned npm packages when installed version does not match", async () => {
			const installedPath = join(tempDir, ".omk", "npm", "node_modules", "example");
			mkdirSync(installedPath, { recursive: true });
			writeFileSync(join(installedPath, "package.json"), JSON.stringify({ name: "example", version: "1.0.0" }));
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".omk", "npm", "node_modules", "example");
