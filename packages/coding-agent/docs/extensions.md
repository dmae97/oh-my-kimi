> omk can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend omk's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.
...
- **Custom commands** - Register commands like `/mycommand` via `omk.registerCommand()`
- **Session persistence** - Store state that survives restarts via `omk.appendEntry()`
...
Create `~/.omk/agent/extensions/my-extension.ts`:
...
| `.omk/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as omk packages, see [packages.md](packages.md).
...
For distributed omk packages installed with `omk install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (omk: ExtensionAPI) {
  // Subscribe to events
  omk.on("event_name", async (event, ctx) => {
  ...
  omk.registerTool({ ... });
  omk.registerCommand("name", { ... });
  omk.registerShortcut("ctrl+x", { ... });
  omk.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, omk awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `omk.registerProvider()` are flushed.
...
export default async function (omk: ExtensionAPI) {
  ...
  omk.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `omk --list-models`.
...
thinking level changes (settings, keybinding, omk.setThinkingLevel())
...
```typescript
omk.on("resources_discover", async (event, _ctx) => {
...
```typescript
omk.on("session_start", async (event, ctx) => {
...
```typescript
omk.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, omk emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
omk.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, omk emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
omk.on("session_compact", async (event, ctx) => {
...
omk.on("session_tree", async (event, ctx) => {
...
```typescript
omk.on("session_shutdown", async (event, ctx) => {
...
```typescript
omk.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data OMK uses to build the system prompt. This lets you inspect what OMK has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
```typescript
omk.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

omk.on("turn_end", async (event, ctx) => {
...
```typescript
omk.on("message_start", async (event, ctx) => {
  // event.message
});

omk.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

omk.on("message_end", async (event, ctx) => {
...
```typescript
omk.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

omk.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

omk.on("tool_execution_end", async (event, ctx) => {
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports OMK's system prompt string rather than the final serialized provider payload.

```typescript
omk.on("before_provider_request", (event, ctx) => {
...
```typescript
omk.on("after_provider_response", (event, ctx) => {
...
```typescript
omk.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `omk.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, omk waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "open-multi-agent-kit";
...
```typescript
import { isBashToolResult } from "open-multi-agent-kit";

omk.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "open-multi-agent-kit";

omk.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap omk's built-in local bash backend
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while omk is idle.
...
Request a graceful shutdown of omk.
...
Returns OMK's current system prompt string.
...
Returns the base inputs OMK currently uses to build the system prompt.
...
```typescript
import { SessionManager } from "open-multi-agent-kit";

omk.registerCommand("switch", {
...
```typescript
omk.registerCommand("reload-runtime", {
...
export default function (omk: ExtensionAPI) {
  ...
  omk.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      omk.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### omk.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### omk.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`omk.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `omk.getAllTools()` and are callable by the LLM without `/reload`.

Use `omk.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
### omk.sendMessage(message, options?)
...
### omk.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
omk.sendUserMessage("What is 2+2?");
...
omk.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
omk.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### omk.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
omk.setSessionName("Refactor auth module");
```
...
```typescript
const name = omk.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### omk.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
omk.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
omk.setLabel(entryId, undefined);
...
### omk.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, omk keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "omk-tui";
...
### omk.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for messages with your `customType`. See [Custom UI](#custom-ui).

### omk.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
omk.registerShortcut("ctrl+shift+p", {
...
### omk.registerFlag(name, options)

Register a CLI flag.

```typescript
omk.registerFlag("plan", {
...
### omk.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await omk.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### omk.getActiveTools() / omk.getAllTools() / omk.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools.

```typescript
const active = omk.getActiveTools();
const all = omk.getAllTools();
// [{
//   name: "read",
...
omk.setActiveTools(["read", "bash"]); // Switch to read-only
```
...
if (model) {
  const success = await omk.setModel(model);
  if (!success) {
...
### omk.getThinkingLevel() / omk.setThinkingLevel(level)
...
```typescript
const current = omk.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh"
omk.setThinkingLevel("high");
```
...
```typescript
omk.events.on("my:event", (data) => { ... });
omk.events.emit("my:event", { ... });
```

### omk.registerProvider(name, config)

Register or override a model provider dynamically. Useful for proxies, custom endpoints, or team-wide model configurations.

Calls made during the extension factory function are queued and applied once the runner initialises. Calls made after that — for example from a command handler following a user setup flow — take effect immediately without requiring a `/reload`.

If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. omk waits for the factory before startup continues, so the registered models are available immediately, including to `omk --list-models`.
...
omk.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});
...
```typescript
omk.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    omk.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `omk.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `omk.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "open-multi-agent-kit";
...
omk.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via omk.exec (captured from extension closure)
    const result = await omk.exec("some-command", [], { signal });
...
**Important:** Use `StringEnum` from `omk-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when omk resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "open-multi-agent-kit";
...
For `user_bash`, extensions can reuse omk's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
export default function (omk: ExtensionAPI) {
  ...
  omk.registerTool({ name: "db_connect", ... });
  omk.registerTool({ name: "db_query", ... });
  omk.registerTool({ name: "db_close", ... });
...
```typescript
import { keyHint } from "open-multi-agent-kit";
...
// Terminal title
ctx.ui.setTitle("omk - my-project");
...
```typescript
import { Text, Component } from "omk-tui";
...
import { matchesKey } from "omk-tui";
...
omk.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `omk.sendMessage()`:
...
```typescript
import { highlightCode, getLanguageFromPath } from "open-multi-agent-kit";
