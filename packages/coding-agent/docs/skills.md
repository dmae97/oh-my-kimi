> omk can create skills. Ask it to build one for your use case.
...
- Packages: `skills/` directories or `omk.skills` entries in `package.json`
- Settings: `skills` array with files or directories
- CLI: `--skill <path>` (repeatable, additive even with `--no-skills`)

Discovery rules:
- In `~/.omk/agent/skills/` and `.omk/skills/`, direct root `.md` files are discovered as individual skills
...
For project-level Claude Code skills, add to `.omk/settings.json`:
...
|-------|----------|-------------|
| `name` | Yes | Max 64 chars. Lowercase a-z, 0-9, hyphens. Unlike the standard, OMK does not require this to match the parent directory because that standard requirement is suboptimal for shared skill directories. |
...
OMK does not require the name to match the parent directory. The Agent Skills standard does, but that requirement is suboptimal for shared skill directories used by multiple tools.
...
OMK validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
...
- [OMK Skills](https://github.com/badlogic/omk-skills) - Web search, browser automation, Google APIs, transcription
