...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `open-multi-agent-kit` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "open-multi-agent-kit";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown OMK and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what OMK loaded without re-discovering resources. */
...
/** Configuration for registering a provider via the OMK extension API registerProvider() method. */
...
export interface ProviderModelConfig {
	...
	reasoning: boolean;
	/** Maps OMK thinking levels to provider/model-specific values; null marks a level unsupported. */
...
export type ExtensionFactory = (omk: ExtensionAPI) => void | Promise<void>;
...
export interface Extension {
	...
	/** Unsubscribe callbacks for extension event subscriptions, disposed when the extension is reloaded. */
