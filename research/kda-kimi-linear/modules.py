...
class KimiDeltaAttention(nn.Module):
    def __init__(self, d_model, n_heads, d_head=None, conv_size=4, chunk_size=64, gate_bias=1.0):
        super().__init__()
        self.h = n_heads
        ...
        self.a = nn.Linear(d_model, self.inner, bias=True)     # per-channel forget-gate logits
        ...
        nn.init.constant_(self.a.bias, gate_bias)              # start near alpha≈1 (long memory)
    ...
    def forward(self, x, state=None):
        ...
        alpha = torch.exp(-F.softplus(self._heads(self.a(x)))) # per-channel gate in (0,1)
