...
Each task MUST include core OMK Execution Metadata (`role`, `deps`, `files`, `verify`, `gate`, and `risk`). Every non-`not applicable` harness metric MUST have an explicit verification task that additionally includes `metric`, a paired baseline, `target`, `regression-floor`, and sanitized `evidence`.
...
- [ ] T001 [P] Description with exact file paths in `backticks`
  ...
  > metric: [harness dimension; omit this metric block for ordinary tasks]
  > baseline: [paired version/artifact]
  > target: [measurable result]
  > regression-floor: [minimum result]
  > verify: `npm test` or `npm run check`
  > evidence: [sanitized artifact path]
...
- **metric / baseline / target / regression-floor / evidence**: Required together only for a harness-metric verification task; carry one metric row from the spec and plan.
- **verify**: Exact command to run after task completion. OMK runs this as a command-pass gate.
- **evidence**: Sanitized result artifact; never raw secrets, prompts, source, tool output, environment values, PII, or absolute user paths.
