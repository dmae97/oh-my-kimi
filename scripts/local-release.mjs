...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "omk-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `omk-${platform}.zip` : `omk-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createOmkShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "omk.cmd"))) {
			writeFileSync(join(installDirectory, "omk.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\omk.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "omk.ps1"), '& "$PSScriptRoot/node_modules/.bin/omk.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "omk.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\omk.exe" %*\r\n');
		writeFileSync(join(installDirectory, "omk.ps1"), '& "$PSScriptRoot/node_modules/.bin/omk.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "omk"), join(installDirectory, "omk"));
}
...
if (rootPackageJson.name !== "omk-monorepo") {
...
if (!options.skipInstall) {
	...
	createOmkShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createOmkShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `omk-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "omk.exe" : "omk")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "omk.cmd" : "omk")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "omk.cmd" : "omk")} --help`);
	}
}
