...
const agentDirEnvName = "OMK_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "OMK_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific OMK_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force OMK_OFFLINE=1 / OMK_SKIP_VERSION_CHECK=1
...
async function runBuild() {
	process.stdout.write("Building packages/tui, packages/ai, packages/protocol, packages/agent, and packages/coding-agent...\n");
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "omk-startup-benchmark-")) : undefined;
