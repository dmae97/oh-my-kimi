...
PROJECT = os.getcwd()
SKILL_ROOTS = [
    os.path.join(HOME, ".omk/agent/skills"),
    os.path.join(HOME, ".claude"),
    os.path.join(HOME, ".pi/agent"),
    os.path.join(PROJECT, ".omk/skills"),
]
SKIP = {
    "node_modules",
    ".git",
...
def locate_skill_dirs():
    ...
    def walk(d, depth):
        ...
        try:
            ...
            return
        for e in entries:
            if e.name in SKIP or (
                e.name.startswith(".") and e.name not in (".claude", ".agents")
            ):
                continue
            try:
                is_dir = e.is_dir()
...
def build():
    ...
    active_skills = {
        ...
        if n["type"] == "skill" and n.get("tier") == "active"
    }
    ...
    for sk in active_skills:
        ...
        for sp in scripts:
            ...
            rows.append(
                {
                    "path": os.path.relpath(sp, HOME),
                    "lang": lang,
                    "loc": loc,
                    "deps": sorted(deps),
                    "project_local": sp.startswith(PROJECT),
                }
            )
...
def main():
    ...
    if "--agent" in args:
        ...
        print(
            f"  code-backed skills: {len(sks)}  scripts: {scripts}  LOC: {loc}  distinct deps: {len(deps)}"
        )
    ...
    L += [
        "",
        "## Agent code footprint (top 25 by scripts)",
        "",
            ...
            json.dump(
                {
                    ...
                    "dep_blast": {d: sorted(s) for d, s in dep_skills.items()},
                },
                f,
                indent=1,
            )
    ...
    print(
        f"  code-backed skills: {code_backed}/{len(active_skills)}  scripts: {total_scripts}  LOC: {total_loc}"
    )
    print(
        f"  distinct deps: {len(dep_skills)}  project-local scripts (KG-covered): {proj_local}"
    )
    print(
        "  top deps:",
        ", ".join(
            f"{d}({len(s)})"
            for d, s in sorted(dep_skills.items(), key=lambda x: -len(x[1]))[:6]
        ),
    )
